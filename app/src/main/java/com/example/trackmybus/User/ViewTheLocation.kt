...
import com.example.trackmybus.databinding.ActivityUserMainBinding
...
class ViewTheLocation : AppCompatActivity(),OnMapReadyCallback {
    ...
    val ff= arrayOf(android.Manifest.permission.ACCESS_FINE_LOCATION,
...
maps.getMapAsync(this)
        val reques= registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()){
...
fused.lastLocation.addOnSuccessListener {
    if(it!=null){
        val latlon=LatLng(decimal.format(it.latitude).toDouble(),decimal.format(it.longitude).toDouble())
        googleMap?.addMarker(MarkerOptions().title("Current Location").position(latlon))
        googleMap?.moveCamera(CameraUpdateFactory.newLatLngZoom(latlon,16f))

        val buslatlon=intent.getStringExtra("latlon")?.split(",")
        if(buslatlon?.size==2){
            val secondlatlon=LatLng(buslatlon[0].toDouble(),buslatlon[1].toDouble())
            googleMap?.addMarker(MarkerOptions().title("Bus Location").position(secondlatlon))
            googleMap?.addPolygon(PolygonOptions().add(secondlatlon,latlon).fillColor(Color.RED).strokeColor(Color.RED))
        }else{
