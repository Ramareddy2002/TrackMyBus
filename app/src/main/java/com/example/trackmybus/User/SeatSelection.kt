...
import com.example.trackmybus.Responses.GetSeats
import com.example.trackmybus.Responses.Model.Search
import com.example.trackmybus.Responses.ReTrofit
import com.example.trackmybus.databinding.ActivitySeatSelectionBinding
...
class SeatSelection : AppCompatActivity(), View.OnClickListener {
        ...
        bind.book.setOnClickListener {
            if (selected.isNotEmpty()) {
                Intent(this, Billing::class.java).apply {
                    ...
                    putExtra("routedetails2", routedetails)
    ...
    private fun getdata(routeid: String?,selectDate: String) {
        ...
        if (routeid != null) {
            ...
            ReTrofit.instance.getAllseats(condition = "gettoday", routeid = "$routeid", date = selectDate).enqueue(/* callback = */
                ...
                val k=response.body()?.apply {
                    ...
                    seats.split(",").forEach { the ->  if(the.trim().isNotEmpty()) {
